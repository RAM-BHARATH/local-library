...
exports.book_delete_get = function(req, res, next) {
    async.parallel({
        ...
        book_instances(callback) {
            BookInstance
            .find({ book: req.params.id }).exec(callback)
        }
    }, function (err, results){
            if(err) { return next(err); }
    ...
    async.parallel({
        book_instances(callback) {
            ...
            if(results.book_instances.length>0){
                // Book has book instances. Render same way as for GET requests
                res.render('book_delete', { title: 'Delete Book', bookinstances: results.book_instances });
                return;
            }
            else{
                // Book has no book instances. Delete object and redirect to the list of authors.
                // bookid is obtained from the form in book_delete
